...
class Workbook {
     ...
     * @param {boolean} [opts.excludeDocMetadata] - Whether to exclude the docMetadata folder from the output.
     * @returns {Promise<string|Uint8Array|ArrayBuffer|Blob|Buffer>} The data.
     */
    outputAsync(opts) {
        opts = opts || {};
        ...
        // Remove docMetadata folder if option is set.
        if (opts.excludeDocMetadata) {
            Object.keys(this._zip.files).forEach(filePath => {
                if (filePath.startsWith('docMetadata/')) {
                    this._zip.remove(filePath);
                }
            });
        }
